...
def stratified_k_means_constrained(X, n_clusters, stratifying_var, size_min=None, size_max=None, init='k-means++',
                        n_init=10, max_iter=300, verbose=False,
                        tol=1e-4, random_state=None, copy_x=True, n_jobs=1,
                        return_n_iter=False):
    """K-Means clustering with minimum and maximum cluster size constraints and .
    ...
    stratifying_var : array-like, shape (n_samples, )
        Variable along which the stratification should be conducted.
    ...
    if not (len(stratifying_var) == X.shape[0]):
        raise ValueError('Stratifing variable of diffrent size than X')
    ...
    if n_jobs == 1:
        ...
        for it in range(n_init):
            # run a k-means once
            labels, inertia, centers, n_iter_ = stratified_kmeans_constrained_single(
                X, n_clusters, stratifying_var=stratifying_var,
        ...
        results = Parallel(n_jobs=n_jobs, verbose=0)(
            delayed(stratified_kmeans_constrained_single)(X, n_clusters, stratifying_var=stratifying_var,
                                               size_min=size_min, size_max=size_max,
...
def stratified_kmeans_constrained_single(X, n_clusters, stratifying_var, size_min=None, size_max=None,
    ...
    for i in range(max_iter):
        ...
        labels, inertia = \
            _stratified_labels_constrained(X, centers, stratifying_var, size_min, size_max, distances=distances)
...
def _stratified_labels_constrained(X, centers, stratifying_var, size_min, size_max, distances):
    ...
    edges, costs, capacities, supplies, n_C, n_X = stratified_minimum_cost_flow_problem_graph(X, C, D, stratifying_var, size_min, size_max)
...
def stratified_minimum_cost_flow_problem_graph(X, C, D, stratifying_var, size_min, size_max):
    ...
    # Stratifing sizes
    cluster_group_sizes = (pd.Series(stratifying_var, name='total_size')
                           .value_counts()
                           .to_frame()
                           .assign(mean_size=lambda x: x['total_size'] / C.shape[0])
                           .assign(min_size=lambda x: np.floor(x['mean_size']))
                           .assign(max_size=lambda x: np.where(x['mean_size'] == x['min_size'],
                                                               x['min_size'], x['min_size'] + 1)))

    # Create indices of nodes
    n_X = X.shape[0]
    n_C = C.shape[0]
    n_groups = cluster_group_sizes.shape[0]
    X_ix = np.arange(n_X)
    C_dummy_ix = np.arange(X_ix[-1] + 1, X_ix[-1] + 1 + n_C * n_groups)
    C_dummy_groups = np.array_split(C_dummy_ix, n_groups)
    C_ix = np.arange(C_dummy_ix[-1] + 1, C_dummy_ix[-1] + 1 + n_C)
    art_ix = C_ix[-1] + 1

    # Edges
    edges_X_C_dummy = [cartesian([X_ix[stratifying_var == x_index], c_index])
                       for x_index, c_index
                       in zip(cluster_group_sizes.index, C_dummy_groups)]
    edges_X_C_dummy = np.vstack(edges_X_C_dummy)
    edges_X_C_dummy = edges_X_C_dummy[edges_X_C_dummy[:, 1].argsort()]
    edges_X_C_dummy = edges_X_C_dummy[edges_X_C_dummy[:, 0].argsort(kind='mergesort')]

    edges_C_dummy_C = np.stack([C_dummy_ix, np.tile(C_ix, n_groups)],
                               axis=1)  # Each C' connects to a corresponding C (centroid)
    edges_C_art = np.stack([C_ix, art_ix * np.ones(n_C)], axis=1)
    ...
    capacities_C_dummy_C = np.repeat(cluster_group_sizes['max_size'], n_C)
    cap_non = n_X
    ...
    supplies_C = -1 * size_min * np.ones(n_C)
    supplies_art = -1 * (n_X - n_C * size_min)
    supplies = np.concatenate([
        supplies_X,
        np.zeros(n_C * n_groups),  # C_dummies
        supplies_C,
        [supplies_art]
    ])
...
class StratifiedKMeansConstrained(KMeans):
    ...
    def fit(self, X, stratifying_var, y=None):
        ...
        self.cluster_centers_, self.labels_, self.inertia_, self.n_iter_ = \
            stratified_k_means_constrained(
                X, n_clusters=self.n_clusters,
                stratifying_var=stratifying_var,
    ...
    def predict(self, X, stratifying_var, size_min='init', size_max='init'):
        """
    ...
    def fit_predict(self, X, stratifying_var, y=None):
        ...
        labels : array, shape [n_samples,]
            Index of the cluster each sample belongs to.
        """
        return self.fit(X, stratifying_var).labels_

#
#
# # wczytanie i przygotowanie danych
# account = pd.read_excel('KOS.xlsx')
#
# name = 'Damian Niewiarowski'#'Sebastian Sienkiewicz', 'Grzegorz Podbielski', 'Damian Niewiarowski'
#
# account = account[account['PH primary name'] == name]  #'Sebastian Sienkiewicz', 'Grzegorz Podbielski'
# account = account[(account['Latitude'] != 'Not assigned')|(account['Longitude'] != 'Not assigned')]
# account['Latitude'] = account['Latitude'].astype('float')
# account['Longitude'] = account['Longitude'].astype('float')
#
# eda_graph = account[account['PH primary name'] == name][['Latitude','Longitude','PH Freq']]
#
...
#     random_state=None,
#     copy_x=True,
#     init = np.array(eda_graph.iloc[[3, 20, 70, 90, 100], :][['Latitude', 'Longitude']]),
#     n_jobs=1)
#
# labels = clf.fit_predict(eda_graph[['Latitude', 'Longitude']], stratifying_var = np.array(eda_graph['PH Freq']))
#
# print(pd.DataFrame({'labels': labels, 'var': eda_graph['PH Freq'], 'dummy': 1}).groupby(['labels','var'], as_index=False).sum())